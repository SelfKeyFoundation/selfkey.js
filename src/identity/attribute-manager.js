...
export class AttributeManager {
		...
		let { matchedAttributes, matchedRequirements } = requirements.reduce(
			(acc, curr) => {
				...
				if (typeof curr === 'string') {
					curr = { schemaId: curr, required: true };
				}
				...
				if (curr.id) {
					...
					acc.matchedRequirements.push({ attr, requirement: curr });
					if (attr) {
				...
				attr =
					attributes.find(
						attr =>
							attr.schemaId === curr.schemaId &&
							!attr.id &&
							!acc.matchedAttributes.has(attr)
					) || null;
			...
			{ matchedRequirements: [], matchedAttributes: new Set() }
		);
		// handle attributes without matching requirement
		if (matchedAttributes.size < attributes.length) {
			const withoutRequirements = attributes
				.filter(attr => !matchedAttributes.has(attr))
				.map(attr => ({ attr, requirement: null }));
	...
	validateOneAttribute(attr, requirement) {
		...
		const { valid, errors } = repository.validateData(schemaId, data);
